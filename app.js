document.addEventListener('DOMContentLoaded', () => {
    ...
    const APP_CONFIG = {
        ...
        // NOTE:
        // 这里请填你的 Cloudflare Worker 代理地址（例如： https://你的worker名.workers.dev/api/proxy ）
        // 前端仍然保留 aiPool 的“新增/切换 API”逻辑，但实际请求会走 Worker 来绕开 CORS。
        aiProxyEndpoint: localStorage.getItem('pca_ai_proxy_endpoint') || 'https://picasso-ai-proxy.1063528814.workers.dev/'
    };
    ...
    // 确保 activeApiId 有效
    if (!State.activeApiId || State.activeApiId === 'default') {
        if (State.aiPool.length > 0) {
            State.activeApiId = State.aiPool[0].id;
    ...
    const HOT_TECHS = [
        ...
        {
            id: 't6', name: '神经形态计算 SNN', hot: 74, trend: 'flat',
            status: '基于脉冲神经网络的非冯·诺依曼架构正在复兴，旨在实现比现有 GPU 快两个数量级且功耗降低 90% 的端侧前向推理。',
            val: '这将从根本上改写嵌入式设备（如可穿戴、隐形传感器）的算力逻辑，使"万物即 AI"不再受制于电池续航。',
            news: [
                { title: "英特尔 Loihi 2 芯片实测：在边缘视觉识别任务中展现降维级能效比", hot: 78, url: "https://www.intel.com" },
                { title: "大脑模拟：SNN 为何被认为是通用人工智能 AGI 的节能终极解法？", hot: 68, url: "https://www.bing.com" },
                { title: "深度专访：解析国内类脑计算独角兽的技术路线图与量产暗礁", hot: 62, url: "https://www.bing.com" }
            ]
        }
    ];
    ...
    // 回显代理端点
    if (UI.settingProxyUrl) UI.settingProxyUrl.value = APP_CONFIG.aiProxyEndpoint;

    // 优先从本地缓存读取资讯流，实现情报持续累积
    const cachedFeed = localStorage.getItem('pca_feed');
    if (cachedFeed) {
        MockFeed = JSON.parse(cachedFeed);
    } else {
    ...
    if (lastOpen !== today) {
        ...
        // 缓存已通过上方逻辑加载，无需在此重置为 MOCK_NEWS_DATA
    ...
    async function callDeepSeek(prompt, onChunk = null) {
        if (!APP_CONFIG.aiProxyEndpoint) {
            const msg = 'AI 代理地址未配置：请先在浏览器设置 `localStorage.pca_ai_proxy_endpoint` 为你的 Cloudflare Worker URL。';
            console.error(msg);
            UI.radarStatus.innerHTML = `<span style="color:#ef4444"><i class="ph ph-warning-circle"></i> ${msg}</span>`;
            return msg;
        }
        ...
        for (let api of tryPool) {
            // 如果名称包含 "内置" 或 "代理"，则允许在未配置前端 key 的情况下尝试（适配 Worker 已内置环境变量的场景）
            const isManaged = api.name.includes('内置') || api.name.includes('代理');
            if (!api.key && !isManaged) continue;

            try {
                let apiBase = api.url.trim().replace(/\/$/, "");

                // [URL 智能补全与容错]
                if (apiBase.toLowerCase() === 'gemini') {
                    apiBase = 'https://generativelanguage.googleapis.com/v1beta';
                }
                if (apiBase.includes('googleapis.com') && !apiBase.includes('/v1')) {
                    apiBase += '/v1beta'; // 自动补齐版本号
                }

                const isGemini = apiBase.includes('googleapis') || api.name.toLowerCase().includes('gemini');
                ...
                if (isGemini) {
                    // === GOOGLE GEMINI PROTOCOL (AI Studio Final Fix) ===
                    const modelName = api.model || 'gemini-1.5-flash';
                    const fullUrl = `https://generativelanguage.googleapis.com/v1/models/${modelName}:generateContent?key=${api.key}`;
                    headers['x-target-url'] = fullUrl;
                    
                    response = await fetch(APP_CONFIG.aiProxyEndpoint, {
                        method: 'POST',
                        headers: headers,
                        body: JSON.stringify({
                            contents: [{
                                parts: [{ text: `你是一位顶级 AI 产业产品经理。回复必须一针见血，禁止废话。请求：\n\n${prompt}` }]
                            }]
                        })
                    });
                } else {
                    ...
                    response = await fetch(APP_CONFIG.aiProxyEndpoint, {
                        ...
                        body: JSON.stringify({
                            ...
                            messages: [
                                { role: 'system', content: '你是一位极其犀利、一针见血的顶端 AI 产业产品经理兼投资人。回复必须一针见血，不许客套。' },
        ...
        const detailedError = lastError?.message || "未知响应错误";
        UI.radarStatus.innerHTML = `<span style="color:#ef4444"><i class="ph ph-warning-circle"></i> 故障诊断：${detailedError}</span>`;
        return simulateLocalAI(`[全线链路故障诊断报告]\n当前所有引擎均已尝试，最后拦截到的错误为：\n"${detailedError}"\n\n💡 专家排查建议：\n1. 请检查 Key 是否已失效或余额不足；\n2. 确认是否已通过 CORS 插件解除跨域；\n3. 检查模型名称（如 gemini-1.5-flash）是否正确。`);
    }
    ...
    function renderRadar() {
        ...
        UI.hotCards.innerHTML = HOT_TECHS.map(t => {
            return `
                <div class="metric-card" id="card-${t.id}" onclick="openDetail('${t.id}', '${t.name}', 'hot')">
        ...
        if (State.watchedTechs.length === 0) {
            UI.watchCards.innerHTML = '<span class="empty-tip">点击 + 追踪技术</span>';
        } else {
            UI.watchCards.innerHTML = State.watchedTechs.map(t => {
                return `
                    <div class="metric-card" id="card-${t.id}" onclick="openDetail('${t.id}', '${t.name}', 'watch')">
                        <span class="mc-name">${t.name}</span>
                        <span class="mc-hot">🔥${t.hot}</span>
                    </div>
                `;
    ...
    function bindEvents() {
        ...
        // CF Proxy Save
        UI.btnSaveProxy.addEventListener('click', () => {
            const url = UI.settingProxyUrl.value.trim();
            if (url) {
                APP_CONFIG.aiProxyEndpoint = url;
                localStorage.setItem('pca_ai_proxy_endpoint', url);
                alert('代理端点配置成功！');
            }
        });
        ...
        UI.cancelNewApi.addEventListener('click', () => {
            ...
            UI.newApiName.value = UI.newApiKey.value = '';
        });

        function deriveApiFromName(name) {
            const n = (name || '').toLowerCase();
            if (n.includes('deepseek')) return { url: 'https://api.deepseek.com/v1', model: 'deepseek-chat' };
            if (n.includes('openai')) return { url: 'https://api.openai.com/v1', model: 'gpt-4o-mini' };
            if (n.includes('gemini')) return { url: 'gemini', model: 'gemini-1.5-flash' };
            return null;
        }
        ...
        UI.saveNewApi.addEventListener('click', () => {
            ...
            if (name && key) {
                const derived = deriveApiFromName(name);
                if (!derived) {
                    alert('仅支持从名称自动补齐：请在“端点名称”中包含 `DeepSeek` / `OpenAI` / `Gemini` 之一。');
                    return;
                }
                const newApi = { id: 'api_' + Date.now(), name, url: derived.url, key, model: derived.model };
    ...
    function saveFeed() {
        // 限制本地缓存数量（如保留最近50条），防止 localStorage 溢出并保持性能
        if (MockFeed.length > 50) MockFeed = MockFeed.slice(0, 50);
        localStorage.setItem('pca_feed', JSON.stringify(MockFeed));
    }
